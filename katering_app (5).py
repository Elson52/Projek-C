...
def menu_pengguna():
    ...
    status_pemesanan = st.session_state.get('status_pemesanan', None)
    if status_pemesanan:
        st.write(f"Status Pemesanan Anda: {status_pemesanan}")
    ...
    if st.button("Pesan"):
        # Tambahkan logika untuk memproses pesanan di sini
        # ... (kode untuk menambahkan pesanan ke daftar_pesanan, menampilkan invoice, dll.) ...
        harga_makanan = {"Nasi Goreng": 15000, "Ayam Penyet": 20000, "Sate": 25000}
        total_harga = harga_makanan[makanan] * jumlah
        st.success("Pesanan berhasil dibuat!")  # Tampilkan pesan sukses
